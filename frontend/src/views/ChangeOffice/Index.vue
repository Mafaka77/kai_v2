  ...
  <MainLayout>
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 class="text-2xl font-bold text-slate-900">Change Office</h1>
          <p class="text-slate-500 mt-1 text-sm">Request transfer/posting to a different office and track request status.</p>
        </div>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div v-if="loading" class="flex flex-col items-center justify-center py-24 text-slate-400">
          ...
          <span class="text-sm font-medium">Loading your requests...</span>
        </div>
        ...
        <div v-else-if="requests.length === 0" class="py-24 text-center text-slate-400">
          ...
          <p class="text-slate-900 font-bold text-lg">No Office Change Requests</p>
          <p class="text-slate-400 text-sm mt-1 max-w-sm mx-auto">
            You have not submitted any office transfer requests yet. Click "New Change Request" to apply.
          </p>
        </div>
            ...
            <table class="w-full border-collapse text-left">
              <thead>
                <tr class="bg-slate-50 border-b border-slate-100">
                  ...
                  <th class="text-slate-500 font-semibold text-xs uppercase px-6 py-4.5 tracking-wider">Remark / Note</th>
              ...
              <tbody class="divide-y divide-slate-100">
                <tr v-for="req in requests" :key="req.id" class="hover:bg-slate-50/50 transition-colors">
                  <td class="px-6 py-4">
                    <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-indigo-50 text-indigo-700 border border-indigo-200 font-bold text-xs rounded-lg">
                  ...
                  <td class="px-6 py-4 text-xs text-slate-600 italic">
                    {{ req.remark || 'No remark specified' }}
                  </td>
                  <td class="px-6 py-4 text-xs text-slate-500 font-medium">
                    {{ formatDate(req.createdAt) }}
                  </td>
                  <td class="px-6 py-4 text-center">
                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border shadow-xs" :class="statusBadge(req.status)">
      ...
      <div v-if="showModal" class="fixed inset-0 bg-slate-900/40 backdrop-blur-xs z-50 flex items-center justify-center p-4">
        <div class="bg-white w-full max-w-md rounded-2xl shadow-2xl p-6 space-y-4">
          <div class="flex items-center justify-between border-b border-slate-100 pb-3">
            <div>
              <h3 class="text-base font-bold text-slate-900">Request Office Transfer</h3>
              <p class="text-xs text-slate-400 mt-0.5">Select a new office to apply for posting transfer.</p>
            </div>
          ...
          <form @submit.prevent="submitRequest" class="space-y-4">
            <div>
              ...
              <SearchableSelect
                ...
                placeholder="Search & select target office..."
                required
              />
            </div>

            <div>
              <label class="block font-semibold text-slate-700 text-xs mb-1.5">Reason / Remark</label>
              <textarea
                v-model="form.remark"
                rows="3"
                placeholder="State your reason for transfer request..."
                class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs placeholder-slate-400 text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 resize-none"
              ></textarea>
            </div>
...
import { useChangeOfficeStore } from '../../stores/changeOffice'

const store = useChangeOfficeStore()
...
const submitRequest = () => store.submitRequest()
